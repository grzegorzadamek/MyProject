...
import * as parts from 'src/datas/parts.json';
import * as blackList from 'src/datas/blackList.json';
import { TranslateService } from '@ngx-translate/core';

@Component({
  selector: 'my-schedule-root',
  templateUrl: './my-schedule.component.html',
  styleUrls: ['./my-schedule.component.scss'],
})
export class MyScheduleComponent {
  public url: string = '';
  public partList: { name: string; begin: string; end: string }[] = (
    parts as any
  ).default;
      ...
      .subscribe(
        (result) => {
          ...
          this.partList.map((part) => {
            let section: string = this._page
              ?.split(part.begin)
              .pop()
              ?.split(part.end)[0] as string; // get sections of meeting
            this._partInArray = [];
            ...
            if (part.name === 'intro') {
              ...
              let patterns = [
                '<strong>s*(.*?)s*</strong></h3>',
                '<strong>s*(.*?)s*</strong></span></h3>',
              ];
            ...
            this.tasks.push({
              name: part.name,
              label: this._sectionLabelOK,
              parts: this._partInArray,
            });
          });
        },
        (error) => {
